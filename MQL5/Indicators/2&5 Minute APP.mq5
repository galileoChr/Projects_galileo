...
void myAlert(string type, string message)
  {
   if(type == "print")
     ...
     {
      Print(type+" | 2&5 Minute APP @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
   else if(type == "order")
     {
     }
     ...
     {
      if(Audible_Alerts) Alert(type+" | 2&5 Minute APP @ "+Symbol+","+IntegerToString(Period())+" | "+message);
      if(Send_Email) SendMail("2&5 Minute APP", type+" | 2&5 Minute APP @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
      if(Push_Notifications) SendNotification(type+" | 2&5 Minute APP @ "+Symbol()+","+IntegerToString(Period())+" | "+message);
     }
  }
...
int OnInit()
  {   
   ...
   MA_handle = iMA(NULL, PERIOD_CURRENT, 4, 0, MODE_SMA, PRICE_CLOSE);
   if(MA_handle < 0)
     {
