...
// Update the EA at every period below:
   ...
   if(CheckNewBar(PERIOD_M5) != 1)
   ...
   else
     {
      ...
      int randNumber = 0 + 5*MathRand()/32768; // 3-5

      //Comment("randNumber: "+randNumber);

      // DCA_Pivot Strategy breakdown: .....

      int strat_DCA_Pivot = DCA_Pivot_Strategy_Breakdown(PivotBufferPrices(indicator_handlePivot_D1_vis_to_M5,PERIOD_M5,4),
                            PivotBufferPrices(indicator_handlePivot_D1_vis_to_M5,PERIOD_M5,1),
                            PivotBufferPrices(indicator_handlePivot_D1_vis_to_M5,PERIOD_M5,0),rates);

      Comment("strat DCA with Pivot Prices: "+strat_DCA_Pivot);

//      if(strat_DCA_Pivot == 1)
//        {
//          Buy
//         trade.Buy(0.01,Symbol(),Ask,0,Ask + 10*_Point);
//        }
//
//      if(strat_DCA_Pivot == 0)
//        {
//         Sell
//         trade.Sell(0.01,Symbol(),Bid,0,Bid - 10*_Point);
//        }
...
//+------------------------------------------------------------------+
//|This strategy verifies if cuurent price is above pivot point or not. It
//|makes decisions based on that. Sell if above pivot and buy if below pivot
//+------------------------------------------------------------------+
int DCA_Pivot_Strategy_Breakdown(double pivot_D1_S1, double pivot_D1_R1, double pivot_D1_PivotPrice, MqlRates &currentPrice[])
  {
   if(currentPrice[1].close > pivot_D1_PivotPrice && currentPrice[1].close < pivot_D1_R1)
     {
      // Sell
      return 0;
     }
   else
      if(currentPrice[1].close < pivot_D1_PivotPrice && currentPrice[1].close > pivot_D1_S1)
        {
         return 1;
        }
